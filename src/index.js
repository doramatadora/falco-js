...
const lint = file => falco(['-json', file])

const lintText = (text, { vclFileName, autoAddIncludes = true, diagnosticsOnly = true, deserialize = true } = {}) =>
  withTempFile(async file => {
    const falcoFlags = ['-json']
    if (vclFileName) {
    ...
    if (autoAddIncludes) {
      ...
      for (const [_, includePath] of text.matchAll(/include\s+\"([^"]*\/[^"]+)\"/gm) || []) {
